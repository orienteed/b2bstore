...
import {
  useOrderIncidenceBtn
} from '../../talons/OrderIncidence/useOrderIncidenceBtn';
    ...
    return indices.map((value, key) => {
      return (
        <div className={classes.incidenceRow}>
          ...
          <FileUploader handleChange={(file)=>{handleChange(file, key)}} name={"image["+key+"]"}  types={fileTypes} />
        </div>
      );
    });
    ...
    <div className={classes.OrderIncidenceDiv}>
      <Popup
        trigger={
          <button
            type="button"
            className={[classes.OrderIncidenceBtn, classes.root].join(" ")}
          >
            {"OrderIncidence"}
          </button>
        }
