...
import defaultOperations from '@magento/peregrine/lib/talons/WishlistPage/wishlistItem.gql.js';
...
const deriveOptionSelectionsFromProduct = item => {
    const optionSelections = new Map();
    for (const { id, value_id } of item.configurable_options) {
        optionSelections.set(id.toString(), value_id);
    }
...
export const useWishlistItem = props => {
    ...
    const optionSelections = useMemo(
        () => deriveOptionSelectionsFromProduct(item),
        [item]
    );

    const itemImage = findMatchingVariant({
        optionCodes,
        optionSelections,
        variants
    });
    ...
    const isInStock = stockReal ? (stockReal !== 'OUT_OF_STOCK' ? true : false) : (stockStatus !== 'OUT_OF_STOCK' ? true : false);
