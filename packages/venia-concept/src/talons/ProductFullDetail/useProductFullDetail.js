...
export const useProductFullDetail = props => {
    ...
    const { error: errorWishList, loading: loadingWishList, data: dataWishList } = useQuery(
        operations.getCustomerWishList,
        { fetchPolicy: 'cache-and-network' }
    );

    const wishListId = useMemo(() => {
        return dataWishList ? dataWishList.customer.wishlists[0].id : null;
    }, [dataWishList]);
    ...
    const wishListItems = useMemo(() => {
        return dataWishListItem ? dataWishListItem.customer.wishlist_v2.items_v2.items : null;
    }, [dataWishListItem]);
