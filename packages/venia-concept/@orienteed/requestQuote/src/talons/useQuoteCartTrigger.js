...
import { MP_QUOTE, DELETE_ITEM_FROM_MP_QUOTE, GET_CUSTOMER } from '@orienteed/requestQuote/src/query/requestQuote.gql';
...
export const useQuoteCartTrigger = props => {
    ...
    useMemo(() => {
        if(custoemrData && custoemrData.customer){
            const {customer:{mp_quote_id}}=custoemrData
    ...
    const hideQuoteCartTrigger = DENIED_MINI_CART_ROUTES.includes(
    ...
    useEffect(() => {
        if (data != undefined) {
            const { mpQuote: { quote } } = data
            setMyQuote(quote)
        }
    }, [data]);

    useState(()=>{
        window.addEventListener(AFTER_UPDATE_MY_REQUEST_QUOTE, async function(event){
            setIsLoading(true)
    ...
    const handleDeleteItem = useCallback(async (itemId) => {
        ...
        const {data:{deleteItemFromMpQuote:{quote}}} = await deleteItemFromMpQuote({
            variables: {
