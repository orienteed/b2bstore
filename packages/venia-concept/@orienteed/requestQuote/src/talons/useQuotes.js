...
export const useQuotes = (props) => {
  ...
  // Handle Delete Page
  const handleDeleteQuote = useCallback(async (value) => {
    await setIsLoading(true)
  ...
  // Handle cancel Page
  const handleCancelQuote = useCallback(async (value) => {
    await setIsLoading(true)
    await cancelMpQuote({
      variables: {
        quoteId: parseInt(value)
  ...
  const handleDuplicateQuote = useCallback(async (value) => {
    await setIsLoading(true)
    const {data:{duplicateMpQuote:{quote}}} = await duplicateMpQuote({
      variables: {
        quoteId: parseInt(value)
      }
    })
  ...
  const handleQuoteToCart = useCallback(async (value) => {
    await setIsLoading(true)
