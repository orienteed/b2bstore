...
export const createProductVariants = product => {
	...
	if (product && product.configurable_options) {
		...
		const cartesian = (...array) =>
			array.reduce((array, current) => array.flatMap(cur => current.map(n => [cur, n].flat())));

		const configurableOptionsValueIndexes = product.configurable_options.map(option =>
			option.values.map(value => value.value_index)
		);
		...
		for (let i = 0; i < len; i++) {
			...
			for (const option of variantsValueIndexes) {
				...
				foundMatch =
					option.length > 1
						? Array.from(currentValueIndex).sort().toString() === option.sort().toString()
			...
			if (currentValueIndex.length && currentValueIndex.length > 1) {
				...
				const code = product.configurable_options.find(option =>
					option.values.find(value => value.value_index === currentValueIndex)
				);
				newAttributes.push({
