...
export const useCompanyAccountUsers = () => {
    ...
    const submitAddUsers = useCallback(
        async formValues => {
            ...
            // console.log({ userRoles }, { mpca_role_id });
            const role = userRoles?.mpCompanyAccountsUserRoles.find(({ role_id }) => role_id == mpca_role_id);

            if (modalType === 'addUser') {
                try {
                    console.log({ mpca_role_id });
                    await createNewUser({
                        variables: {
                            password,
                            input: {
                                mpca_role_id: mpca_role_id?.value ? mpca_role_id.value : Number(mpca_role_id),
                                mpca_role: {
                                    name: role.name
                                },
                                ...rest
