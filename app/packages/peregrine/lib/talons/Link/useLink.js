...
const useLink = (props, passedOperations = {}) => {
	...
	const [runQuery, { called: pageTypeCalled }] = useLazyQuery(resolveUrlQuery);
	const linkPath = shouldPrefetch ? resourceUrl(to) : null;

	useEffect(() => {
		if (!shouldPrefetch || pageTypeCalled || !runQuery || !elementRef.current || !intersectionObserver) {
			return;
		}
	...
	}, [shouldPrefetch, elementRef, pageTypeCalled, linkPath, intersectionObserver, runQuery]);
