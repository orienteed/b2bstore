...
export const useShippingMethod = props => {
	...
	const hasData =
		data && data.cart.shipping_addresses.length && data.cart.shipping_addresses[0].selected_shipping_method;
	...
	const dispatchEvent = useCallback(
		shippingMethod => {
			dispatch({
				type: !isUpdateMode ? 'CHECKOUT_SHIPPING_METHOD_ADDED' : 'CHECKOUT_SHIPPING_METHOD_UPDATED',
				payload: {
					cart_id: cartId,
