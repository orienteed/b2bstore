...
const ImageShimmer = props => {
	...
	const dimensions = useMemo(() => {
		...
		if (window.matchMedia('(max-width: 48rem)').matches && mobileImage && mobileImage.dimensions) {
		...
		if (figureRef.current) {
			...
			if (dimensions.width > width) {
				if (window.matchMedia('(max-width: 48rem)').matches && mobileImage && mobileImage.dimensions) {
					...
					dimensions.height = dimensions.width * mobileImage.dimensions.ratio;
				} else if (desktopImage && desktopImage.dimensions) {
					dimensions.width = width;
					dimensions.height = dimensions.width * desktopImage.dimensions.ratio;
				}
			}
		}
