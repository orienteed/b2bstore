...
const CmsDynamicBlockGroup = props => {
	...
	return items.map((item, index) => <DynamicBlock key={item.uid} index={index} {...item} />);
};
...
CmsDynamicBlockGroup.propTypes = {
	displayMode: oneOf([DISPLAY_MODE_FIXED_TYPE, DISPLAY_MODE_SALES_RULE_TYPE, DISPLAY_MODE_CATALOG_RULE_TYPE]),
	locations: array,
