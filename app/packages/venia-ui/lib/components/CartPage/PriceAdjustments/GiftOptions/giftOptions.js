...
const GiftOptions = props => {
	...
	const { allow_order, allow_gift_receipt, allow_printed_card } = props?.giftOptionsConfigData || {};
				...
				{savingOptions.includes(giftReceiptProps.field) ? (
					<div className={classes.savingContainer}>
						<span className={classes.savingText}>
							<FormattedMessage id={'giftOptions.saving'} defaultMessage={'Saving'} />
						</span>
	...
	const includeGiftMessage =
		allow_order === '1' ? (
			<>
				...
				<div className={showGiftMessageResult ? classes.giftMessageResultContainer : classes.hidden}>
					<div className={classes.giftMessageResult} data-cy="GiftOptions-giftMessageResult">
						<p>
							<FormattedMessage
					...
					>
						...
						<span className={classes.actionLabel}>
							<FormattedMessage id="giftOptions.editGiftMessageButton" defaultMessage="Edit" />
						</span>
					</LinkButton>
				</div>

				<div className={!showGiftMessageResult ? classes.giftMessageFields : classes.hidden}>
					<Field
						id="to"
						label={formatMessage({
							id: 'giftOptions.to',
					...
					>
						<TextInput {...cardFromProps} data-cy="GiftOptions-cardFrom" />
					</Field>
					<Field
						id="message"
						label={formatMessage({
					...
					<div className={classes.giftMessageActions}>
						{hasGiftMessage ? (
							<Button {...cancelGiftMessageButtonProps} data-cy="GiftOptions-cancelGiftMessageButton">
								<FormattedMessage id="giftOptions.cancelGiftMessageButton" defaultMessage="Cancel" />
							</Button>
						) : null}
						<Button {...saveGiftMessageButtonProps} data-cy="GiftOptions-updateGiftMessageButton">
							{hasGiftMessage ? (
								<FormattedMessage
								...
								<FormattedMessage id="giftOptions.addGiftMessage" defaultMessage="Add Message" />
							)}
						</Button>
					</div>
				</div>
				...
				<div className={classes.checkboxContainer}>
					<Checkbox
						...
						label={formatMessage(
							...
							{
								price:
									printedCardPrice && printedCardPrice.value > 0 ? (
										<>
											{' ( + '}
											<Price
	...
	return (
		<Form data-cy="GiftOptions-form" {...optionsFormProps} className={classes.root}>
