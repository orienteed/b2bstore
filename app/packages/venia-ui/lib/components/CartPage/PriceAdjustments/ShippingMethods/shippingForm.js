...
const ShippingForm = props => {
	...
	const { errors, handleOnSubmit, handleZipChange, isSetShippingLoading } = talonProps;
	...
	return (
		<Fragment>
			<h3 className={classes.formTitle}>
				<FormattedMessage id={'shippingForm.formTitle'} defaultMessage={'Destination'} />
			</h3>
			<FormError errors={Array.from(errors.values)} />
			<Form className={classes.root} initialValues={selectedShippingFields} onSubmit={handleOnSubmit}>
				<Country data-cy="ShippingMethods-ShippingForm-country" validate={isRequired} />
				<Region data-cy="ShippingMethods-ShippingForm-region" validate={isRequired} />
				<Postcode
					fieldInput="zip"
