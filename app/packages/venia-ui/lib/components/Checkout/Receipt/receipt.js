...
const Receipt = props => {
	...
	const { handleCreateAccount, handleViewOrderDetails, isSignedIn } = talonProps;
	...
	const content = isSignedIn ? (
		<Fragment>
			...
			<Button onClick={handleViewOrderDetails}>{viewOrderButtonText}</Button>
		</Fragment>
	) : (
		<Fragment>
			<hr />
	...
	const orderConfText = 'You will receive an order confirmation email with order status and other details.';
