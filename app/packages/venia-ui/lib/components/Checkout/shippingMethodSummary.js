...
const ShippingMethodSummary = props => {
	...
	const [{ shippingAddress, shippingMethod, shippingTitle }] = useCheckoutContext();

	const hasShippingAddress = !!shippingAddress && !isObjectEmpty(shippingAddress);

	const hasShippingMethod = !!shippingMethod;

	const className = hasShippingAddress ? classes.informationPrompt : classes.disabledPrompt;
