...
const WishlistConfirmRemoveProductDialog = props => {
	const { hasError, isOpen, isRemovalInProgress, onCancel, onConfirm } = props;
	...
	const maybeError = hasError ? (
		<p className={classes.errorMessage}>
			<FormattedMessage
				...
				defaultMessage={'There was an error deleting this product. Please try again later.'}
			/>
		</p>
	) : null;
	...
	return (
		<Dialog
			...
			confirmTranslationId={'wishlistConfirmRemoveProductDialog.confirmButton'}
			isModal={true}
			isOpen={isOpen}
			onCancel={onCancel}
			onConfirm={onConfirm}
		...
		>
			<div className={classes.root}>
				...
				<span className={classes.prompt}>
					<FormattedMessage
						id={'wishlistConfirmRemoveProductDialog.confirmationPrompt'}
						defaultMessage={'Are you sure you want to delete this product from the list?'}
					/>
				</span>
			</div>
		</Dialog>
