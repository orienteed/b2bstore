...
const Dropzone = props => {
    ...
    const onDrop = useCallback(
        (attachedFiles, rejectedFiles) => {
            ...
            if (attachedFiles.length > 0) {
                attachedFiles.map(file => {
                    ...
                    reader.onload = e => {
                        if (filesUploaded.filter(({ name }) => name === file.name).length === 0) {
        ...
        [setFilesUploaded, setDropzoneError, filesUploaded]
    );
    ...
    return (
        ...
        >
            ...
            <img
                src={attachFilesIcon}
                className={isTicketClosed ? classes.attachFilesIconDisabled : classes.attachFilesIcon}
                alt="Attach"
            />
        </section>
    );
