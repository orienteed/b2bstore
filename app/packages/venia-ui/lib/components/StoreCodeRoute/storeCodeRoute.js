...
const StoreCodeRoute = () => {
	...
	useEffect(() => {
		if (storeCodeInUrl && storeCodeInUrl !== currentStoreCode) {
			...
			storage.setItem('store_view_currency', storeCurrencies[storeCodeInUrl]);
			storage.setItem('store_view_secure_base_media_url', storeSecureBaseMediaUrl[storeCodeInUrl]);

			// We're required to reload the page as the basename doesn't
			// change entirely without a full page reload.
			history.go(0);
		}
	}, [currentStoreCode, history, storeCodeInUrl, storeCurrencies, storeSecureBaseMediaUrl]);
