...
import defaultClasses from './CustomQuantityStepper.module.css';

export const CustomQuantityStepper = props => {
    const { initialValue, itemId, label, min, onChange, message } = props;
    const { formatMessage } = useIntl();
    const classes = useStyle(defaultClasses, props.classes);
    const iconClasses = { root: classes.icon };

    const talonProps = useCustomQuantityStepper({
        initialValue,
        min,
        onChange
    });
...
CustomQuantityStepper.defaultProps = {
    min: 0,
    initialValue: 1,
    onChange: () => {}
};
