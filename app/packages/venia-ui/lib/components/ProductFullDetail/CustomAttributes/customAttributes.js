	...
	const list = useMemo(
		() =>
			customAttributes.reduce((previousAttribute, currentAttribute) => {
				const usedInComponents = currentAttribute.attribute_metadata?.used_in_components || [];
				// Visible on front attributes only
				if (usedInComponents.includes(IS_VISIBLE_ON_FRONT)) {
					const attributeContent = (
						<li key={currentAttribute.attribute_metadata.uid} className={classes.listItem}>
