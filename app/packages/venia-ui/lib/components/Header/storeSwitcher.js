...
const StoreSwitcher = props => {
	...
	storeGroups.forEach((group, key) => {
		...
		group.forEach(({ storeGroupName, storeName, isCurrent, storeCode }) => {
			...
			stores.push(
				...
				>
					<SwitcherItem active={isCurrent} onClick={handleSwitchStore} option={storeCode}>
						{label}
					</SwitcherItem>
				</li>
			);
		...
		groups.push(
			<ul role="listbox" className={classes.groupList} key={key} data-cy="StoreSwitcher-group">
				{stores}
			</ul>
		);
	});
	...
	return (
		<div className={classes.root} data-cy="StoreSwitcher-root">
			...
			<div ref={storeMenuRef} className={menuClassName} data-cy="StoreSwitcher-menu">
