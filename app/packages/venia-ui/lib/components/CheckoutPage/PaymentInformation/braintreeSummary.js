...
const BraintreeSummary = props => {
	...
	const { billingAddress, isBillingAddressSame, isLoading, paymentNonce } = useBraintreeSummary();
	...
	const billingAddressSummary =
		!isBillingAddressSame && billingAddress ? (
			<div className={classes.address_summary_container}>
				<div>
					<span className={classes.first_name}>{billingAddress.firstName}</span>
					<span className={classes.last_name}>{billingAddress.lastName}</span>
				</div>
				<div>
					<span className={classes.street1}>{billingAddress.street1}</span>
					<span className={classes.street2}>{billingAddress.street2}</span>
					<span className={classes.city}>{billingAddress.city}</span>
					<span className={classes.state}>{billingAddress.state}</span>
				</div>
				<div>
					<span className={classes.postalCode}>{billingAddress.postalCode}</span>
					<span className={classes.country}>{billingAddress.country}</span>
				</div>
			</div>
		) : null;
	...
	return (
		<div className={classes.root}>
			<div className={classes.heading_container}>
				...
				>
					<Icon size={16} src={EditIcon} classes={{ icon: classes.edit_icon }} />
			...
			<div className={classes.card_details_container}>
				<span className={classes.payment_type}>
					<FormattedMessage id={'global.creditCard'} defaultMessage={'Credit Card'} />
				</span>
				<span className={classes.payment_details}>{paymentSummary}</span>
			</div>
