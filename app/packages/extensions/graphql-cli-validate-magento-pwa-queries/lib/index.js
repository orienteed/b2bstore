...
const plugin = {
	...
	DESCRIPTION: 'Validate all query files and template literals in a project against a schema.',
	SUPPORTED_ARGUMENTS: {
		project: {
			alias: 'p',
...
async function validateQueries(context, argv) {
	try {
		...
		console.log(`Validating GraphQL queries in ${chalk.blue(project)} project...`);
		...
		if (!schemaExists) {
			throw new Error(
				`Could not find a schema at ${schemaPath}.\n Run \`graphql get-schema --project ${project}\` to download the schema before running ${plugin.COMMAND}.`
			);
		}
		...
		context.spinner.succeed(`Found ${queryFiles.length} files to test in ${filesGlob}`);
...
function getErrorResolutionDetails() {
	...
	if (packageVersion) {
		versionSpecificDetails = `This Venia store is from PWA Studio version ${chalk.green(packageVersion)}.`;
	...
	return `${chalk.red(`Your versions of PWA and Magento may be incompatible.`)}
