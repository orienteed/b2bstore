...
export default original => props => {
	...
	const { data: storefrontData, ready: storefrontDataReady, errors } = useExtensionContext();
			...
			import('@adobe/magento-storefront-events-sdk').then(mse => {
				...
				if (orgId && datastreamId) {
					...
					mse.context.setStorefrontInstance({
						...
						storeViewCurrencyCode: storefrontContext.store_view_currency_code,
						catalogExtensionVersion: storefrontContext.catalog_extension_version
					});
