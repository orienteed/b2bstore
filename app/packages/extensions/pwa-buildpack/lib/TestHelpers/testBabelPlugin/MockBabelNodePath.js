...
class MockBabelNodePath {
	...
	get(dotPath) {
		let result;
		try {
			result = dotPath.split('.').reduce((node, segment) => node[segment], this.node);
		} catch (e) {
			throw new Error(`Path ${dotPath} not found in node: ${inspect(this.node)}`);
		}
		if (!result || typeof result !== 'object') {
			return result;
		}
		if (Array.isArray(result)) {
			return result.map(node => new MockBabelNodePath(this._source, this._options, node));
		}
