...
const { getModuleRules, getSpecialFlags, getResolveLoader } = require('../WebpackTools/configureWebpack');
...
async function buildModuleWith(
	...
	const hasFlag = await getSpecialFlags(otherWebpackSettings.special || {}, bus, resolver);
	...
	if (!webpackConfig.plugins.some(plugin => plugin instanceof BuildBusPlugin)) {
