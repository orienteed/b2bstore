...
async function makeDirFromNpmPackage(packageName) {
	...
	try {
		...
		throw new Error(`Invalid template: could not get tarball url from npm: ${e.message}`);
	}
	...
	try {
		...
		throw new Error(`Invalid template: could not download tarball from NPM: ${e.message}`);
	}
...
module.exports.describe = 'Create a PWA project in <directory> based on template.';
...
module.exports.builder = yargs =>
	yargs
		...
		.group(['template', 'backendUrl', 'backendEdition', 'braintreeToken'], 'Project configuration:')
		.options({
			template: {
				describe:
			...
			backendUrl: {
				alias: 'b',
				describe: 'URL of the Magento 2.3 instance to use as a backend. Will be added to `.env` file.'
			},
			backendEdition: {
				describe: 'Edition of the magento store (Adobe Commerce or Magento Open Source)'
			},
			braintreeToken: {
				describe:
...
module.exports.handler = async function buildpackCli(argv) {
	...
	if (params.backendUrl) {
		...
		if (backendUrl && backendUrl !== params.backendUrl) {
			...
			Object.assign(process.env, magentoNS.toEnv({ backendUrl: params.backendUrl }));
		}
	}
	...
	if (params.backendEdition) {
		...
		if (backendEdition && backendEdition !== params.backendEdition) {
			...
			Object.assign(process.env, magentoNS.toEnv({ backendEdition: params.backendEdition }));
		}
	}
	...
	if (params.braintreeToken) {
		...
		if (braintreeToken && braintreeToken !== params.braintreeToken) {
			...
			Object.assign(process.env, checkoutNS.toEnv({ braintreeToken: params.braintreeToken }));
		}
	}
	...
	const showCommand = command => ' - ' + chalk.whiteBright(`${params.npmClient} ${command}`);
	...
	const prerequisiteNotice =
		prerequisiteCommand.length > 0
			? `- ${chalk.whiteBright(prerequisiteCommand)} before running the below commands.`
			: '';
	prettyLogger.warn(`Created new PWA project ${params.name}. Next steps:
    ${prerequisiteNotice}
    ${showCommand(
		customOriginCommand
	)} to generate a unique, secure custom domain for your new project. ${chalk.greenBright('Highly recommended.')}
    ${showCommand('run watch')} to start the dev server and do real-time development.
    ${showCommand('run storybook')} to start Storybook dev server and view available components in your app.
    ${showCommand('run build')} to build the project into optimized assets in the '/dist' directory.
    ${showCommand('start')} after build to preview the app on a local staging server.
