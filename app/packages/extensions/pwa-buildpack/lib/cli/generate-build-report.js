...
const removeDuplicateBackends = backendEnvironments => uniqBy(backendEnvironments, 'url');

async function fetchSampleBackendUrls() {
	try {
		const res = await fetch('https://fvp0esmt8f.execute-api.us-east-1.amazonaws.com/default/getSampleBackends');
		const { sampleBackends } = await res.json();

		return removeDuplicateBackends([...sampleBackends.environments, ...defaultSampleBackends.environments]).map(
			({ url }) => url
		);
	} catch {
...
function inspectDependencies(pkg) {
	...
	const dependenciesToScan = [...Object.keys(pkg.dependencies), ...Object.keys(pkg.devDependencies)].filter(
		dependency => dependency.startsWith('@magento/') || dependency.startsWith('@adobe/')
	);
	...
	if (existsSync(path.resolve(cwd, 'yarn.lock'))) {
		...
		prettyLogger.log(`Found ${dependencies.size} @magento dependencies in yarn.lock`);
	} else if (existsSync(path.resolve(cwd, 'package-lock.json'))) {
		// using npm
		dependencies = getNpmDependencies(dependenciesToScan);
		prettyLogger.log(`Found ${dependencies.size} @magento dependencies in package-lock.json`);
	} else {
		throw new Error(
			...
			.forEach(yarnPkgName => {
				...
				if (dependencyMap.has(packageName)) {
					if (!dependencyMap.get(packageName).includes(version)) {
						dependencyMap.set(yarnPkgName, [...dependencyMap.get(packageName), version]);
					}
				} else {
...
function getNpmDependencies(dependencies) {
	...
	dependencies.map(packageName => {
		...
		if (dependencyMap.has(packageName)) {
			if (!dependencyMap.get(packageName).includes(version)) {
				dependencyMap.set(packageName, [...dependencyMap.get(packageName), version]);
			}
		} else {
			dependencyMap.set(packageName, [version]);
		}
		...
		if (depConfig.dependencies) {
			Object.keys(depConfig.dependencies).map(depConfigDepName => {
				if (dependencies.includes(depConfigDepName)) {
					const version = depConfig.dependencies[depConfigDepName].version;

					if (dependencyMap.has(depConfigDepName)) {
						if (!dependencyMap.get(depConfigDepName).includes(version)) {
							dependencyMap.set(depConfigDepName, [...dependencyMap.get(depConfigDepName), version]);
						}
					} else {
...
async function buildpackCli() {
	...
	prettyLogger.info(`Generating build report for ${pkg.name}@${pkg.version}. This may take a moment.`);
	prettyLogger.log('\n');
...
module.exports.describe = 'Generates a report of general build information that can be used when debugging an issue.';
