...
module.exports.handler = async function buildpackCli({ directory }) {
	...
	try {
		...
		prettyLogger.info(
			`Acquired custom hostname and SSL cert for ${hostname}. Development server will run on port ${ports.development} and staging server will run on port ${ports.staging}.`
		);
	} catch (e) {
