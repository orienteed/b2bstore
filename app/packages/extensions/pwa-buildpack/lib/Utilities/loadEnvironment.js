...
class EnvironmentInvalidError extends Error {
	constructor(validationErrors) {
		...
		if (missingVarsOutput.length) {
			output.push(`Missing required environment variables:\n${missingVarsOutput.join('\n')}`);
		}
...
This call to loadEnvironment() will assume that the working directory ${context} is project root.`
	...
	for (const section of definitions.sections) {
		for (const variable of section.variables) {
			...
			if (typeof typeFac !== 'function') {
				throw new Error(
					`Bad environment variable definition. Section ${section.name} variable ${JSON.stringify(
						variable,
						null,
						1
	 ...
	 */
	try {
		const loadedEnv = envalid.cleanEnv(compat.env, envalidValidationConfig, {
...
function parseEnvFile(dir, log) {
	...
	try {
		...
		debug(`Using environment variables from ${path.relative(process.cwd(), envPath)}: %s`, parsed);
	} catch (e) {
		if (e.code === 'ENOENT') {
			return false;
		} else {
