...
module.exports = async function serve(dirname) {
	...
	if (customHttpsSettings.key && customHttpsSettings.cert) {
		...
		if (existsSync(key) && existsSync(cert)) {
			prettyLogger.info('Custom key and cert paths provided, creating HTTPS server.');
			const ssl = {
			...
			prettyLogger.warn('Custom key and cert paths provided but files not found, creating HTTP server.');
		}
	}
	 ...
	 */
	if (process.env.PORT && process.env.PORT !== 'null' && process.env.PORT !== 'undefined') {
		prettyLogger.info(`PORT is set in environment: ${process.env.PORT}`);
		envPort = process.env.PORT;
	} else if (stagingServerSettings.port) {
		prettyLogger.info(`STAGING_SERVER_PORT is configured: ${stagingServerSettings.port}`);
	...
	if (config.isProd) {
		...
		if (customOrigin.enabled) {
			try {
				...
				prettyLogger.error('Could not configure or access custom host. Using loopback...', e.message);
			}
		}
	}
