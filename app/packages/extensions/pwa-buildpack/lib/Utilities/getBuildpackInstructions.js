...
function getInstruction(rootDir, instructionType) {
	...
	const instructionScriptPath = resolve(rootDir, BUILDPACK_DIR, instructionType);
	try {
		return require(instructionScriptPath);
	} catch (e) {
		throw new Error(
			`Buildpack could not find a valid '${instructionScriptName}' file in ${rootDir}. This file must be present to instruct Buildpack how to use the ${rootDir} package to perform the "${instructionType}" operation. Original error: ${e.message}`
		);
	}
}
