...
class CompatEnvAdapter {
	static isExpired({ dateChanged, warnForDays }) {
		...
		const ttl = Math.min(CompatEnvAdapter.MAX_WARNING_DAYS, daysToWarn) * ONE_DAY_MS; // max is also default
		...
		for (const change of this._sortedChanges) {
			...
			switch (change.type) {
				...
				case 'renamed':
					...
					if (change.supportLegacy && !env.hasOwnProperty(change.update)) {
	...
	warnRemoved(change, env) {
		return `Environment variable ${change.name} has been removed in ${CompatEnvAdapter.RELEASE_NAME}, because ${
			change.reason
		}.\nCurrent value is ${env[change.name]}, but it will be ignored.`;
	}
	warnRenamed(change, env) {
		let logMsg = `Environment variable ${change.name} has been renamed in ${CompatEnvAdapter.RELEASE_NAME}. Its new name is ${change.update}.`;
