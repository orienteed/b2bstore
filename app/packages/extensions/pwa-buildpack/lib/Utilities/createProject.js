...
const getIgnores = packageRoot => {
	try {
		return (
			gitIgnoreToGlob(resolve(packageRoot, createProject.GITIGNORE_FILE), packageRoot)
...
const makeCopyStream = ({ fs, packageRoot, directory, options, ignores, visitor }) =>
	new Promise((succeed, fail) => {
		...
		const copyStream = walk(packageRoot, {
			fs,
			filter: p => !isMatch(relative(packageRoot, p), ignores) && !isMatch(p, ignores)
		});
...
async function createProject(options) {
	...
	const { instructions, packageRoot } = getBuildpackInstructions(template, ['create']);
