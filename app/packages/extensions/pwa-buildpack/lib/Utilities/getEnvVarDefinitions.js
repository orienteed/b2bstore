...
function getEnvVarDefinitions(context) {
	...
	const definitions = JSON.parse(JSON.stringify(require('../../envVarDefinitions.json')));
	try {
		...
		bus.init();
		bus.getTargetsOf('@magento/pwa-buildpack').envVarDefinitions.call(definitions);
		debug('BuildBus for %s augmented env var definitions from buildpack.envVarDefinitions interceptors', context);
	} catch (e) {
		debug('BuildBus for %s errored calling buildpack.envVarDefinitions. Proceeding with base definitions', context);
	}
	return definitions;
}
