...
proto.isNSSInstalled = function () {
	try {
		return devCertUtils.run('brew list -1').toString().includes('\nnss\n');
	} catch (e) {
		return false;
	}
};
...
const alreadyProvisioned = hostname => {
	...
	debug('checking for %s in devcert.configuredDomains() === %o', hostname, configuredDomains);
...
Please enter the password for ${chalk.whiteBright(username)} on ${chalk.whiteBright(os.hostname())}.`)
						);
					} else {
						debug('appears to be a sudo session already');
					}
				} else {
					debug('non-interactive! cleared UI timeout for getCert("%s")', hostname);
			...
			clearTimeout(timeout);
			debug('failure! cleared UI timeout for getCert("%s"): %o', hostname, e);
			reject(e);
		}
	});
}
...
function getUniqueDomainAndPorts(directory, customName, addUniqueHash) {
	...
	if (customName && typeof customName === 'string') {
		...
		try {
			...
			console.warn(
				debug.errorMsg(`Using default "${name}" prefix. Could not autodetect project name from package.json: `)
			);
	...
	const uniquePortOffset = parseInt(Buffer.from(digest, 'base64').toString('hex').slice(-5), 16) % 1000;
...
async function configureHost(options) {
	...
	const { addUniqueHash = true, dir, subdomain, exactDomain, interactive = true } = options;
	const { uniqueSubdomain, ports } = getUniqueDomainAndPorts(dir, exactDomain || subdomain, addUniqueHash);
	let hostname;
	if (exactDomain) {
	...
	try {
		...
		throw Error(debug.errorMsg(`Could not setup development domain: \n${e.stack}`));
	}
}
