...
class BuildBus extends Trackable {
	...
	_getEnvOverrides() {
		...
		this._depsAdditional = envDepsAdditional ? envDepsAdditional.split(',') : [];
	}
	/** @private */
	...
	_getTargets(depName) {
		...
		if (!targetProvider) {
			throw new Error(`${this._identifier}: Cannot getTargetsOf("${depName}"): ${depName} has not yet declared`);
		}
	...
	_requestTargets(requestor, requested) {
		...
		for (const [name, tapable] of Object.entries(targetProvider._tapables)) {
	...
	runPhase(phase) {
		...
		pertaining.forEach(dep => {
			...
			if (!targetProvider) {
				targetProvider = new TargetProvider(this, dep, this._requestTargets);
