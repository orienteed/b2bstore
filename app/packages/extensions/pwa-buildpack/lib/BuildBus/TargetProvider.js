...
const { appearsToBeTapable, getTapableType, types } = require('./mapHooksToTargets');
...
class TargetProvider extends Trackable {
	...
	_linkTarget(requestorName, targetName, tapable) {
		...
		return new TargetClass(this, requestorName, targetName, getTapableType(tapable), tapable);
	}
	/**
	...
	declare(declarations) {
		...
		for (const [targetName, hook] of Object.entries(declarations)) {
			...
			this.own[targetName] = this._linkTarget(this.name, targetName, hook);
		}
	}
	/**
	...
	of(depName) {
		if (this.phase !== 'intercept') {
			this.track(
				...
				{
					type: 'lifecycle',
					message: `ran of(${depName}) in the "${this.phase}" phase. Be sure this is what you want to do; outside the intercept phase, this behavior is not guaranteed.`
				}
			);
		}
		...
		if (!this._intercepted[depName]) {
			this._intercepted[depName] = this._getExternalTargets(this, depName);
		}
