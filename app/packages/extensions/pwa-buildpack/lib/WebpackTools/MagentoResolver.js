...
class MagentoResolver {
	...
	constructor(options) {
		...
		if (!paths || typeof paths.root !== 'string') {
			throw new Error('new MagentoResolver(options) requires "options.paths.root" to be a string');
		}

		const versionExtensions = isAC ? ['.ac.js', '.ee.js'] : ['.mos.js', '.ce.js'];

		const extensions = ['.wasm', '.mjs', ...versionExtensions, '.js', '.jsx', '.json', '.graphql'];
		/** @ignore */
	...
	async resolve(request) {
		return new Promise((res, rej) => {
			try {
				this.myResolver.resolve(this._context, this._root, request, this._requestContext, (err, filepath) => {
					if (err) {
						return rej(err);
					}
					res(filepath);
