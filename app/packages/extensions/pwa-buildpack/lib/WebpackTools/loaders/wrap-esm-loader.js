...
function wrapEsmLoader(content) {
	...
	const uniqueJsId = name =>
		`${Template.toIdentifier(name.includes('/') ? name.slice(name.lastIndexOf('/')) : name)}_${++ids}`;

	const defaultExportRE = /^\s*export\s+default/m;
	// todo make this actually spec with a parser
	const exportRE = name => new RegExp(`^\\s*export\\s+((?:const|let|var|function) )${name}`, 'm');
	...
	for (const { wrapperModule, exportName, defaultExport } of this.query) {
		if (defaultExport) {
			if (!hasDefaultExport()) {
				this.emitWarning(
					`wrap-js-loader: Cannot wrap default export of "${this.resourcePath}" with module "${wrapperModule}" because it does not have a default export.`
				);
			} else {
			...
			this.emitWarning(
				`wrap-js-loader: Cannot wrap export "${exportName}" of "${this.resourcePath}" with module "${wrapperModule}" because it does not have an export named "${exportName}".`
			);
		} else {
			const wrapperIdentifier = addImport(wrapperModule);
	...
	let imports = `// BUILDPACK: wrap-esm-loader added ${importsMap.size} imports
`;
	...
	if (defaultExportWrappers.size > 0) {
		...
		wrappedExports =
			wrappedExports.replace(defaultExportRE, `const ${defaultExportIntermediateVar} = `) +
	...
	if (exportWrappers.size > 0) {
		for (const [exportName, wrappers] of exportWrappers.entries()) {
			...
			wrappedExports =
				wrappedExports.replace(exportRE(exportName), `\n$1${exportIntermediateVar}`) +
