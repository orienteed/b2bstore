...
function exportEsmCollectionLoader(content) {
	...
	if (!defaultExportStartsWith(content, open)) {
		this.emitError(
			new Error(
				`items provided as ${type}, but cannot export an ${type} because template module "${this.resourcePath}" does not export an empty ${type}.
	...
	if (bindings.length === 0) {
		this.emitWarning(new Error(`Bindings array was empty; no changes will be made`));
		return content;
	}
