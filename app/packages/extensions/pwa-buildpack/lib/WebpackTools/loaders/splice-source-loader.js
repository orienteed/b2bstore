...
function spliceSourceLoader(content) {
	return this.query.reduce((source, instr) => {
		...
		const spliceAt = pos => spliceSource(source, pos, instr.insert, instr.remove);
		...
		if (hasRemove && !isNonNegativeInt(instr.remove)) {
			return nope('A "remove" property must be an integer greater than or equal to 0.');
		}

		const found = instructionTypes.filter(type => instr.hasOwnProperty(type));
		if (found.length !== 1) {
			return nope(`Must have one and only one of the options ${inspect(instructionTypes)}.`);
		}
		...
		switch (instructionType) {
			case 'at': {
				if (!isNonNegativeInt(value)) {
					return nope('An "at" property must be an integer greater than or equal to 0.');
				}
				if (value > source.length) {
					return nope(`Out of range: the source is only ${source.length} characters`);
				}
