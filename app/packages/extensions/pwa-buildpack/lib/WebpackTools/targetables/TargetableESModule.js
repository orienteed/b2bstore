...
class TargetableESModule extends TargetableModule {
	...
	addImport(statement) {
		let importStatement =
			statement instanceof SingleImportStatement ? statement : new SingleImportStatement(statement);
		const existingFromSource = this._imports.get(importStatement.source);
		if (existingFromSource && existingFromSource.imported === importStatement.imported) {
		...
		if (this._bindings.has(importStatement.binding)) {
			...
			// different name.
			importStatement = importStatement.changeBinding(this.uniqueIdentifier(importStatement.binding));
		}
	...
	wrapWithFile(exportNameOrWrapperModule, wrapperModule) {
		...
		return this.addTransform('source', '@magento/pwa-buildpack/lib/WebpackTools/loaders/wrap-esm-loader', opts);
	}
}
