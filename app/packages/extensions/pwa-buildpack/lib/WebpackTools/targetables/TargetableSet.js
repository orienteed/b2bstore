...
class TargetableSet {
	...
	constructor(targetProvider) {
		if (!(targetProvider instanceof TargetProvider)) {
			throw new Error('Must supply a TargetProvider to a new TargetableSet.');
		}
	...
	defineEnvVars(sectionName, variableDefs) {
		this._builtins.envVarDefinitions.tap(defs => {
			let mySection = defs.sections.find(section => section.name === sectionName);
			if (!mySection) {
	...
	_bind() {
		this._builtins.transformModules.tapPromise('TargetableSet', async addTransform => {
			for (const [instance, config] of this._connectedFiles.values()) {
				if (typeof config.publish === 'function') {
					await config.publish.call(instance, this._targetProvider.own, instance);
				}
	...
	_provide(Targetable, modulePath, publisher) {
		...
		if (!extant) {
			const targetable = new Targetable(config.module, this._targetProvider);
		...
		throw new Error(
			`Cannot target the file "${modulePath}" using "${Targetable.name}", because it has already been targeted by the ${instance.constructor.name} created by "${this._targetProvider.name}".`
		);
	}
}
