...
class TargetableReactComponent extends TargetableESModule {
	...
	addReactLazyImport(modulePath, localName = 'Component') {
		...
		const elementName = this.uniqueIdentifier('Dynamic' + localName.replace(/[\s-\.,]/g, ''));
		...
		this.insertAfterSource(lazyImportString, `const ${elementName} = reactLazy(() => import('${modulePath}'));\n`);
		return elementName;
	}
	/**
