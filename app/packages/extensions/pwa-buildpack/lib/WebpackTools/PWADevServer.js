...
const { default: playgroundMiddleware } = require('graphql-playground-middleware-express');
...
const PWADevServer = {
	async configure(devServerConfig, webpackConfig) {
		...
		const webpackDevServerOptions = {
			...
			port: devServer.port || (await portscanner.findAPortNotInUse(10000)),
			...
			after(app, server) {
				server.middleware.waitUntilValid(() => {
					...
					try {
						...
						const scheme = isHttpsServer(server.listeningApp) ? 'https://' : 'http://';
						const detectedAddress = server.listeningApp.address();
						const hostname = server.hostname || detectedAddress.address;
						url = new URL(webpackConfig.output.publicPath, scheme + hostname);
						url.port = detectedAddress.port;
					}
					let readyNotice = chalk.green(`PWADevServer ready at \n ${chalk.greenBright.underline(url.href)}`);
		...
		if (customOrigin.enabled) {
			if (devServer.host) {
				...
				console.warn(
					`Custom origins are enabled for this project, but the environment is overriding the custom hostname with ${devServer.host}`
				);
				...
				if (!customOriginConfig) {
					...
					if ((await portscanner.checkPortStatus(requestedPort)) === 'closed') {
