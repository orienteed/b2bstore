	...
	async createMiddleware() {
		...
		const io = {
			async readFile(filepath, enc = 'utf8') {
				...
				debug('compiler.outputFileSystem readFile %s %s', outputBasedPath, enc);
				try {
					return compiler.outputFileSystem.readFileSync(outputBasedPath, enc);
				} catch (e) {
					debug('outputFileSystem %s %s. Trying defaultIO...', outputBasedPath, e.stack);
				}
				...
				try {
					...
					debug('defaultIO %s %s. Trying inputFileSystem...', filepath, e.message);
				}
				...
				try {
					...
					debug('inputFileSystem %s %s. Must throw...', filepath, e.message);
					throw e;
				}
			},
			...
			async networkFetch(path, options) {
				...
				if (protocol === 'https:') {
					return fetch(path, Object.assign({ agent: httpsAgent }, options));
				}
		...
		this.middleware = await upward.middleware(this.upwardPath, this.env, io);
	}
	async getCompiler() {
		if (this.compiler) {
