...
class LocalizationPlugin {
	...
	injectLocalizationLoader() {
		debug('applying InjectPlugin to create global __fetchLocaleData__ for localization import');
			...
			compilation.chunks.forEach(chunk => {
				if (chunk.name && chunk.name.startsWith('i18n')) {
					chunk.getModules().forEach(chunkModule => {
						const chunkLocale = path.parse(chunkModule.resource).name;
						if (locales[chunkLocale] && Array.isArray(locales[chunkLocale])) {
							locales[chunkLocale].forEach(localePath => {
								chunkModule.buildInfo.fileDependencies.add(localePath);
							});
						}
					});
				}
	 ...
	 */
	async writeMergedVirtualLocales(context, locales, inputFileSystem, virtualModules) {
			...
			for (const file of files) {
				...
				if (data) {
					...
					if (translationCount > Object.keys(combined).length) {
						debug(`${file} has ${translationCount - Object.keys(combined).length} override(s).`);
			...
			walk(dir, { fs: inputFileSystem })
				.on('readable', function () {
					...
					while ((item = this.read())) {
						if (item.stats.isFile() && localeFileNameRegex.test(item.path)) {
							...
							debug(
								`Found invalid item within i18n directory: ${item.path}. File names should match locales such as en_US and have a .json extension.`
							);
						}
					}
				})
