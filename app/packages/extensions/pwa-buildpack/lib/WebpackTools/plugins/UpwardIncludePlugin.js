...
class UpwardIncludePlugin {
	...
	async onRun() {
		...
		this.addAsset('upward.yaml', {
			...
			mapping: {
				...
				to: './upward.yml',
				transform: async () => {
					await this.bus.getTargetsOf('@magento/pwa-buildpack').transformUpward.promise(this.definition);
	...
	async getMapping(dir, ref) {
		...
		debug('does fullTargetPath %s start with %s', fullTargetPath, output.path);
		...
		if (await this.isValidResource(fullOriginPath)) {
			debug('%s exists in inputFileSystem. assigning asset %s', fullOriginPath, dir);
			return {
				context: dir,
				from: ref,
				to: ref
	...
	async readUpwardFile(packageDir) {
		...
		try {
			...
			throw new Error(`UpwardIncludePlugin unable to read file ${upwardPath}: ${e.message}`);
		}
		debug(`read ${upwardPath} file successfully`);
		try {
			definition = await jsYaml.safeLoad(yamlTxt);
		} catch (e) {
			throw new Error(`UpwardIncludePlugin error parsing ${upwardPath} contents: \n\n${yamlTxt}`);
		}
