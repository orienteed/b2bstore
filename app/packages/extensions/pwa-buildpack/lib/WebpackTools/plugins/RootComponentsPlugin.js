...
class RootComponentsPlugin {
			...
			walk(dir, {
				...
				.on('readable', function () {
					...
					while ((item = this.read())) {
						if (item.stats.isFile() && extensionRE.test(item.path)) {
	...
	async buildFetchModule() {
		...
		// the webpack build
		const rootComponentsDirsAbs = rootComponentsDirs.map(dir => (isAbsolute(dir) ? dir : join(context, dir)));
		debug('buildFetchModule: absolute root component dirs: %o', rootComponentsDirsAbs);
		const rootComponentImporters = await rootComponentsDirsAbs.reduce(
			async (importersPromise, rootComponentDir) => {
				...
				debug('files from %s: %o', rootComponentDir, rootComponentFiles);
				await Promise.all(
					rootComponentFiles.map(async rootComponentFile => {
						debug('reading file %s', rootComponentFile);
						const rootComponentSource = await this.readFile(rootComponentFile, { encoding: 'utf8' });
						debug('parsing %s source for directives', rootComponentSource.slice(0, 80));
						const { directives, errors } = directiveParser('\n' + rootComponentSource + '\n');
						debug('directive parse found in %s, %o and errors: %o', rootComponentFile, directives, errors);
						if (errors.length) {
							// for now, errors just mean no directive was found
							return;
						}
						debug('filtering %o for RootComponents', directives);
						const rootComponentDirectives = directives.filter(d => d.type === 'RootComponent');
						debug('found %s directives: %o', rootComponentDirectives.length, rootComponentDirectives);
						...
						const { pageTypes, variant } = rootComponentDirectives[0];
						...
						if (!pageTypes || pageTypes.length === 0) {
							...
							pageTypes.forEach(pageType => {
								debug('creating root component map key for %s', rootComponentFile, pageType);
								const key = toRootComponentMapKey(pageType, variant);
								importerSources[
									key
		...
		const rootComponentsMap = `{${rootComponentEntries.map(entry => entry.join(':')).join(',\n')}}`;
