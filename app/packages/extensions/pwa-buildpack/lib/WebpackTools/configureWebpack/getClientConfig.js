...
async function getClientConfig(opts) {
	const { mode, context, paths, hasFlag, vendor, projectConfig, stats, resolver, bus } = opts;
	...
	const config = {
		...
		plugins: [
			...
			new ServiceWorkerPlugin({
				...
				paths,
				injectManifest: true,
				enableServiceWorkerDebugging: !!projectConfig.section('devServer').serviceWorkerEnabled,
	...
	if (mode === 'development') {
			...
			await PWADevServer.configure(
				{
					...
					...projectConfig.sections('devServer', 'imageOptimizing', 'imageService', 'customOrigin'),
		...
		const pwaStudioVersions = {
			...
			...Object.fromEntries(
				Object.entries(packageJson.dependencies || {}).filter(([packageKey]) => packageRegex.test(packageKey))
			),
			...Object.fromEntries(
				Object.entries(packageJson.devDependencies || {}).filter(([packageKey]) =>
					packageRegex.test(packageKey)
				)
			)
		};
		...
		debug(`Unable to verify environment. Cancelling client config creation. Received mode: ${mode}`);
