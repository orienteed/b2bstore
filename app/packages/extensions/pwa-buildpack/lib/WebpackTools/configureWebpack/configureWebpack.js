...
async function validateRoot(appRoot) {
	...
	if (!dirStat.isDirectory()) {
		throw new Error(`Provided application root "${appRoot}" is not a directory.`);
	}
}
...
async function configureWebpack(options) {
	...
	const isAC =
		projectConfig.env.MAGENTO_BACKEND_EDITION === 'AC' || projectConfig.env.MAGENTO_BACKEND_EDITION === 'EE';
	...
	const transforms = new ModuleTransformConfig(resolver, require(path.resolve(context, 'package.json')).name);
