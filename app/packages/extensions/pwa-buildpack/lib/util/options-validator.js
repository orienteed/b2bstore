...
class BuildpackValidationError extends Error {
	constructor(name, callsite, validationErrors) {
		...
		this.message =
			...
			validationErrors.map(([key, requiredType]) => `${key} must be of type ${requiredType}`).join(bullet);
...
module.exports = (name, simpleSchema) => (callsite, options) => {
	const invalid = Object.entries(simpleSchema).reduce((out, [key, requiredType]) => {
