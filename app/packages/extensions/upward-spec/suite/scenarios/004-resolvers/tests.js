...
tape.test('URLResolver extends base URLs producing absolute URLs', async t => {
	...
	const server = await runServer(t, scenarios.getResourcePath('./url-resolver-absolute.yml'));
...
tape.test('URLResolver allows protocol override', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./url-resolver-absolute-unsecure.yml'));

	const base = escape('https://example.com/scope');

	if (server.assert('launched')) {
		try {
			const response = await fetch(new URL('/?id=2&base=' + base, server.url), {
				timeout: process.env.TAP_TIMEOUT || 5000
			});
			await assertOnResponse(t, response, {
				status: 200,
...
tape.test('URLResolver extends base URLs respecting relative paths', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./url-resolver-relative.yml'));
...
tape.test('URLResolver extends base URLs respecting slashes indicating root relative paths', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./url-resolver-root-relative.yml'));
...
tape.test('URLResolver builds urls from templates', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./url-resolver-with-template.yml'), {
	...
	if (server.assert('launched')) {
		try {
			const response = await fetch(new URL(`/?adminApiBase=${base}/`, server.url), {
				timeout: process.env.TAP_TIMEOUT || 5000
			});
			await assertOnResponse(t, response, {
				status: 200,
				...
				text: 'https://admin.host:8081/api/rest/v2/adminToken?refreshToken=a1b2c3&role=owner'
			});
		} catch (e) {
			server.assert('responding', true);
		}
