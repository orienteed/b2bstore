...
tape.test('Static Hello World with only inline deps', async t => {
	...
	const server = await runServer(t, scenarios.getResourcePath('./hello-inline-only.yml'));
...
tape.test('Static Hello World with implicit resolvers', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-inline-implicit-resolvers.yml'));
...
tape.test('Static Hello World with env interpolation', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-env-interpolation.yml'), {
...
tape.test('Static Hello World with env dep and inline template', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-env-inline-template.yml'), {
		ADDRESSEE: 'Terra'
	});
...
tape.test('Static Hello World with env, context, and file template', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-env-context-file-template.yml'), {
		sender: 'planet'
	});
...
tape.test('Static JSON Hello World with template partial resolution', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-context-inline-template-json.yml'), {
...
tape.test('File shortcut resolution', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./hello-file-shortcut.yml'));
	if (server.assert('launched')) {
		const response = await fetch(server.url);
		...
		try {
			...
			t.fail(`swords.csv is valid CSV, but server emitted ${swordsCSV}, which did not parse: ${e.message}`);
		}
