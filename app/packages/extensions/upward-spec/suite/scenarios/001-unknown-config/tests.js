...
tape.test('Crashes if config file is missing', async t => {
	...
	const server = await runServer(t, scenarios.getResourcePath('./absolutely-no-way-this-file-exists'));
...
tape.test('Crashes if config file is unparseable', async t => {
	const scenarios = await gettingScenarios;
	const server = await runServer(t, scenarios.getResourcePath('./unparseable.yml'));
