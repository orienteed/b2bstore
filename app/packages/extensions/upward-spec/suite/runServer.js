...
module.exports = async function runServer(test, configFile, env, customTimeout) {
	let terminator;
			...
			function terminateClean() {
				...
				return new Promise((innerResolve, innerReject) => {
					child.on('close', (_, signal) => {
						flags.running = false;
						return signal === 'SIGKILL' ? innerReject(signal) : innerResolve(signal);
					});
					child.kill('SIGTERM');
				});
			}
			...
			function emitServer() {
				...
				resolve({
					...
					assert(flag, expected = true, msg) {
						...
						const status = flags[flag] === expected ? 'pass' : 'fail';
			...
			child.stdout.on('data', function waitingForUrl(chunk) {
				...
				if (outText.includes('\n')) {
					...
					try {
						...
						reject(new Error(`Could not parse first line of server stdout as a URL: ${e} ${outText}`));
					}
				}
			});
		});
