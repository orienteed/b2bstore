...
module.exports = async (t, response, expected) => {
	...
	if (response.status >= 400 && expected.status !== response.status) {
		...
		return t.fail('Server reported errors: ' + errors.map(({ message }, i) => `[Error ${i + 1}: ${message}]\t`));
	}
	...
	Object.entries(expected.headers).forEach(([header, start]) => {
		...
		if (!value) {
			t.fail(`header ${header} not present in ${inspect(responseHeaders)}`);
		} else {
	...
	try {
		...
		if (expected.contains) {
			if (body.includes(expected.contains)) {
				...
				t.equals(expected.contains + body, body, `body should contain ${expected.contains}`);
			}
		}
		if (expected.text) {
			t.equals(expected.text.trim(), body.trim(), `body should match text ${expected.text.trim()}`);
		}
		if (expected.json) {
			let parsed;
			try {
			...
			Object.entries(expected.json).forEach(([name, value]) =>
				t.equal(value, parsed[name], `JSON ${name} === ${JSON.stringify(parsed[name])}`)
			);
		}
	} catch (e) {
		t.error(e);
