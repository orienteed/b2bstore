...
const config = (api, opts = {}) => {
	...
	const babelKeepAttributes = process.env.BABEL_KEEP_ATTRIBUTES && process.env.BABEL_KEEP_ATTRIBUTES === 'true';

	const removeAttributesPlugin = babelKeepAttributes
		? []
		: [['babel-plugin-react-remove-properties', { properties: ['data-cy'] }]];
	...
	const envConfigs = {
		...
		test: {
			...
			// natively supported.
			plugins: [...plugins, ...removeAttributesPlugin, ['babel-plugin-dynamic-import-node']],
