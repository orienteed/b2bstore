...
class Context {
	static fromRequest(env, request) {
		...
		const url = pick(hostedUrl, ['host', 'hostname', 'port', 'pathname', 'path', 'search', 'searchParams']);
		...
		return new Context({
			...
			request: {
				...
				headerEntries: Object.entries(request.headers).map(([name, value]) => ({ name, value })),
				queryEntries: Object.entries(url.query).map(([name, value]) => ({ name, value }))
			}
		});
	}
			...
			if (!promise) {
				...
				promise = this.visitor.downward([base]).then(value => {
					if (typeof value === 'function') {
						debug('%s assigned to context as function: %o', base, value);
						this.set(base, value);
					} else {
