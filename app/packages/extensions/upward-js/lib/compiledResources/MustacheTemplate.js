...
class MustacheTemplate extends AbstractCompiledResource {
	...
	constructor(...args) {
		...
		if (typeof this.io.readFile !== 'function') {
			throw new Error(`IOInterface missing readFile method: ${inspect(this.io)}`);
		}
		this._loadedPartials = new Map();
	}
	_tryLoadAllExtensions(name, extensions = MustacheTemplate.supportedExtensions) {
	...
	_findUnloadedPartialNames(template) {
		const partialNames = Object.values(template.partials).map(({ name }) => name);
		const uniquePartialNames = [...new Set(partialNames)];
		return uniquePartialNames.filter(name => !this._loadedPartials.has(name));
	}
	async _loadPartial(name) {
		let partial = this._loadedPartials.get(name);
		if (!partial) {
			try {
				partial = Hogan.compile((await this._tryLoadAllExtensions(name)).trim());
	...
	async _loadPartials(partialNames) {
		const loadedPartials = await Promise.all(partialNames.map(name => this._loadPartial(name)));

		const badPartials = loadedPartials.filter(({ badPartial }) => badPartial);
		if (badPartials.length > 0) {
			const partialErrors = badPartials.map(({ badPartial: { name, error } }) => `'${name}: ${error.stack}\n`);
		...
		if (foundDescendentPartials.length > 0) {
			return await this._loadPartials([...new Set(foundDescendentPartials)]);
		}
	}
	async compile() {
		...
		await this._loadPartials(this._findUnloadedPartialNames(this._template));
