...
class IOAdapter {
		...
		return new IOAdapter({
			...
			readFile: (filePath, enc) => {
				...
				if (!containsPath(resolvedPath, baseDir)) {
					throw new Error(`Cannot read ${resolvedPath} because it is outside ${baseDir}`);
				}
	...
	constructor(implementations) {
		const missingImpls = ['readFile', 'networkFetch'].reduce(
			(missing, method) =>
				method in implementations ? missing : missing + `Must provide an implementation of '${method}\n`,
			''
		);
		if (missingImpls) {
