...
class ResolverVisitor {
	...
	async upward(definition, propertyName) {
		...
		if (isPrimitive(defined)) {
			debug('defined: %s is primitive, yielding to context.get("%s")', defined, defined);
	...
	getResolverFor(defined, propertyName) {
		...
		for (Resolver of ResolverList) {
			const recognized = Resolver.recognize && Resolver.recognize(defined);
			if (recognized) {
				return {
		...
		if (defined.resolver) {
			...
			if (!Resolver) {
				this.getResolverFailure = `Unrecognized resolver type: ${defined.resolver}`;
			}
		} else {
			Resolver = ResolverList.find(({ telltale }) => defined.hasOwnProperty(telltale));
			if (!Resolver) {
