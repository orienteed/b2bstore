...
class UpwardMiddleware {
	...
	async getHandler() {
		return async (req, res, next) => {
			...
			let response;
			try {
				response = await buildResponse(this.io, this.env, this.definition, req, this.upwardPath);
				...
				if (isNaN(response.status)) {
					errors.push(`Non-numeric status! Status was '${response.status}'`);
				}
				if (typeof response.headers !== 'object') {
					errors.push(`Resolved with a non-compliant headers object! Headers are: ${response.headers}`);
				}
				if (typeof response.body !== 'string' && typeof response.body.toString !== 'function') {
					errors.push(
...
async function upwardJSMiddlewareFactory(upwardPath, env, io = IOAdapter.default(upwardPath)) {
