	...
	async resolve({ inline }) {
		if (isPrimitive(inline)) {
			...
			const resolvedEntries = await Promise.all(
				Object.keys(inline).map(async key => [key, await this.visitor.upward(inline, key)])
			);
			return fromPairs(resolvedEntries);
		}
		throw new Error(`Internal error: Invalid value supplied to InlineResolver: ${inline}`);
	}
}
