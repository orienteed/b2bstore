...
class UrlResolver extends AbstractResolver {
	...
	async resolve(definition) {
		...
		const argued = [
			...
			.map(async arg => [arg, await this.visitor.upward(definition, arg)]);

		const { baseUrl, pathname, query, search, ...parts } = fromPairs(await Promise.all(argued));
		...
		if (resultUrl.hostname === FAKE_BASE_URL.hostname) {
			...
			const nonRelativeArguments = NON_RELATIVE_PARTS.filter(prop => parts.hasOwnProperty(prop));
			if (nonRelativeArguments.length > 0) {
				die(`Cannot set origin parameters "${nonRelativeArguments}" without setting hostname or baseUrl`);
			}
			// Make a fake root-relative URL, including leading slash.
			const relativeUrl = resultUrl.href.slice(FAKE_BASE_URL.href.length - 1);
			return {
