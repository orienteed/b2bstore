...
module.exports = class TemplateResolver extends AbstractResolver {
	...
	async resolve(definition) {
		...
		if (!definition.provide) {
			...
			if (definition.provide.some(value => typeof value != 'string')) {
				...
				providePromise = Promise.all(
					definition.provide.map(async name => [name, await this.visitor.context.get(name)])
				);
			}
		...
		const [engineName, template, rootEntries] = await Promise.all(toResolve);
		debug('template retrieved, "%s"', template);
		debug(
			'rootEntries retrieved, %o',
			rootEntries.map(([name]) => name)
		);
		...
		if (template instanceof Engine) {
			debug('how nice, the template is already resolved into a %s for us', Engine.name);
			renderer = template;
			...
			throw new Error(
				`Expected string or ${Engine.name}-compatible template and received a foreign object ${template.constructor.name}!`
			);
		}
