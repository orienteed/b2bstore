...
class ServiceResolver extends AbstractResolver {
	...
	async resolve(definition) {
		...
		const toResolve = [
			...
			definition.method ? this.visitor.upward(definition, 'method') : 'POST',
			definition.headers ? this.visitor.upward(definition, 'headers') : {},
			definition.variables ? this.visitor.upward(definition, 'variables') : {}
		];
		...
		return makePromise(execute(link, { query: await parsedQuery.render(), variables }))
			.then(({ data, errors }) => {
