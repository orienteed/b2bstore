...
class FileResolver extends AbstractResolver {
	...
	async resolve(definition) {
		if (!definition.file) {
			throw new Error(`File argument is required: ${JSON.stringify(definition)}`);
		}
		const toResolve = [
			this.visitor.upward(definition, 'file'),
			definition.encoding ? this.visitor.upward(definition, 'encoding') : 'utf8',
			definition.parse ? this.visitor.upward(definition, 'parse') : 'auto'
		];
		const [file, encoding, parse] = await Promise.all(toResolve);
		debug('resolved file %s, encoding %s, parse mode %s', file, encoding, parse);
		...
		if (!allowedencodings.some(value => encoding === value)) {
			throw new Error(`Invalid 'encoding': ${encoding}. Must be one of ${allowedencodings}`);
		}
		...
		if (parse === 'auto') {
			...
			if (!Resource) {
				debug('autoparse found no parser for %s, returning text instead', file);
				return fileText;
			}
		} else {
