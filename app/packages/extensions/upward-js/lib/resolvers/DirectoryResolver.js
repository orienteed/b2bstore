...
class DirectoryResolver extends AbstractResolver {
	...
	async resolve(definition) {
		if (!definition.directory) {
			throw new Error(`Directory argument is required: ${JSON.stringify(definition)}`);
		}
		...
		if (!server) {
			...
			server = serveStatic(path.resolve(path.dirname(this.visitor.upwardPath), directory), staticOpts);
