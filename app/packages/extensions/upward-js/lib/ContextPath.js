...
class ContextPath {
	static from(lookup) {
		...
		if (typeof lookup !== 'string') {
			throw new Error(`Internal error: Cannot build ContextPath from non-string ${lookup}`);
		}
		...
		const path = segments.reduce((parent, newSegment) => parent.extend(newSegment), ContextPath.root);
	...
	getFrom(obj) {
		...
		for (const segment of this._segments) {
			if (Array.isArray(current)) {
				...
				if (!isNaN(index)) {
					...
					throw new Error(`Attempted non-integer lookup on a list: ${current} [${segment}]`);
				}
		...
		debug('traverse %j yielded %j from %j', this._segments, current, obj[this.base()]);
		return current;
	}
	// contains(otherPath) {
