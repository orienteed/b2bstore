...
async function prepare() {
	if (firstRun()) {
		...
		console.warn(
			chalk.green('The issue is being tracked at ') +
				chalk.reset.underline.bold('https://github.com/yarnpkg/yarn/issues/5810') +
				chalk.green(' and this notice will be removed when an upgrade to Yarn fixes this bug.')
		);
	}

	const packages = JSON.parse((await execa('yarn', ['--silent', 'workspaces', 'info'], { cwd: root })).stdout);

	const packagePath = name => path.resolve(root, packages[name].location);

	const buildpackCli = path.resolve(packagePath('@magento/pwa-buildpack'), 'bin', 'buildpack');
	...
	try {
		const { configureHost, loadEnvironment } = require('../packages/pwa-buildpack/lib/Utilities');
		...
		const customOrigin = await loadEnvironment(veniaPath, nullLogger).section('customOrigin');
		if (customOrigin.enabled) {
			...
			if (!customOriginConfig) {
				console.warn(
					chalk.green('Set up a custom origin for your copy of venia-concept:\n\t') +
						chalk.whiteBright(`yarn buildpack create-custom-origin ${veniaPath}`)
				);
			}
		}
	} catch (e) {
